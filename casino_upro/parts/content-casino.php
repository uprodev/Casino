<tr class="campaign-compact-table__offer<?php if($args['index'] == 0) echo ' brand_table_custom_cl' ?>">
	<td class="campaign-compact-table__offer-logo">
		<a href="<?php the_field('url') ?>" target="_blank" rel="nofollow sponsored">
			...
			<p class="campaign-compact-table__offer-brand-name"><?= get_the_title($args['post_id']) ?></p>
		</a>
	...
	<td class="campaign-compact-table__offer-key-features">
		...
		<?php if(have_rows('list', $args['post_id'])): ?>

			<ul>

				<?php while(have_rows('list', $args['post_id'])): the_row() ?>
	...
	<?php if ($field = get_field('rating', $args['post_id'])): ?>
		<td class="campaign-compact-table__offer-rating">
			...
			<a class="campaign-compact-table__offer-brand-name"><?= get_the_title($args['post_id']) ?></a>
		</td>
	<?php endif ?>

	<?php if ($field = get_field('url', $args['post_id'])): ?>
